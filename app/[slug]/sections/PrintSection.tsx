...
<head>
  ...
  <style>
    ...
    .middle-inner { flex: 1; width: 100%; display: flex; flex-direction: column; align-items: flex-start; justify-content: center; text-align: left; gap: 2.2mm; padding-left: 8mm; }
...
export default function PrintSection({ tenant, customers, transporters, defaultCustomer, onPrintDone }: Props) {
  ...
  const pvToBlock = customer ? (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', overflow: 'hidden', paddingLeft: 8 * MM }}>
      {pvCareRow}
      <div style={{ fontSize: sp.toPt * PT, fontWeight: 700, textDecoration: 'underline' }}>To:</div>
      <div style={{ fontSize: sp.coPt * PT, fontWeight: 900, lineHeight: 1.05, wordBreak: 'break-word' }}>{customer.company_name}</div>
      <div style={{ fontSize: sp.adPt * PT, lineHeight: 1.25 }}>{customer.address}</div>
      {customer.contacts.filter((_, i) => selContacts.includes(i)).map((ct, i) => (
        <div key={i} style={{ fontSize: sp.adPt * PT, fontWeight: 600 }}>{ct.name}{ct.phone ? ` : ${ct.phone}` : ''}</div>
      ))}
    </div>
  ) : null
