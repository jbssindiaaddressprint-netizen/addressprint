...
// Adjusted spacing and font sizes to "spread out" large sizes and pack small sizes
interface SP {
  padMm: number     // outer padding
  gapMm: number     // gap pushing the middle To block away from Top/Bottom
  tBarPt: number    // transporter name
  tBarSubPt: number // transporter details
  toPt: number      // "To:" label
  coPt: number      // company name (Huge on A4)
  adPt: number      // address / contact
  fNmPt: number     // "From:" and Company name
  fAdPt: number     // From address
  iconMm: number    // care icon size
  clPt: number      // care label font
  logoMm: number    // Logo max-height
}

const SZ: Record<LabelSize, SP> = {
  'A4':     { padMm: 16, gapMm: 16, tBarPt: 24, tBarSubPt: 14, toPt: 18, coPt: 36, adPt: 18, fNmPt: 16, fAdPt: 14, iconMm: 15, clPt: 6, logoMm: 24 },
  'A5':     { padMm: 10, gapMm: 10, tBarPt: 18, tBarSubPt: 11, toPt: 14, coPt: 26, adPt: 14, fNmPt: 13, fAdPt: 11, iconMm: 12, clPt: 5, logoMm: 18 },
  'A6':     { padMm: 7,  gapMm: 7,  tBarPt: 14, tBarSubPt: 9,  toPt: 11, coPt: 20, adPt: 11, fNmPt: 10, fAdPt: 9,  iconMm: 9,  clPt: 4, logoMm: 14 },
  'A7':     { padMm: 5,  gapMm: 5,  tBarPt: 11, tBarSubPt: 7,  toPt: 9,  coPt: 15, adPt: 9,  fNmPt: 8,  fAdPt: 7,  iconMm: 7,  clPt: 3, logoMm: 10 },
  'DL Env': { padMm: 8,  gapMm: 8,  tBarPt: 12, tBarSubPt: 8,  toPt: 10, coPt: 18, adPt: 10, fNmPt: 9,  fAdPt: 8,  iconMm: 8,  clPt: 4, logoMm: 12 },
  'C5 Env': { padMm: 8,  gapMm: 8,  tBarPt: 14, tBarSubPt: 9,  toPt: 11, coPt: 20, adPt: 11, fNmPt: 10, fAdPt: 9,  iconMm: 9,  clPt: 4, logoMm: 14 },
  'C4 Env': { padMm: 12, gapMm: 12, tBarPt: 18, tBarSubPt: 11, toPt: 14, coPt: 26, adPt: 14, fNmPt: 13, fAdPt: 11, iconMm: 12, clPt: 5, logoMm: 18 },
}
...
const MM = 3.78                 // mm → px roughly
const PT = (25.4 / 72) * MM    // pt → px
...
<head>
  <meta charset="utf-8">
  <style>
    /* Setting margin: 0 combined with strict absolute sizing pins the print 
       to the top-left corner, saving paper when printing small labels on A4 */
    @page { margin: 0; }
    html, body { 
    ...
    /* Vertically center the To block automatically */
    ...
    ${isEnv ? `
      ...
      .bottom {
        position: absolute; left: ${sp.padMm}mm; top: 0; bottom: 0; width: 36%;
  ...
  <div class="sheet">
    ...
    <div class="middle">
      <div class="middle-inner">
        ...
        <div style="font-size: ${sp.adPt}pt; line-height: 1.4; margin-top: 6pt;">${customerAddress}</div>
        <div style="font-size: ${sp.adPt}pt; color: #333; margin-top: 2pt;">${customerPinState}</div>
...
export default function PrintSection({ tenant, customers, transporters, defaultCustomer, onPrintDone }: Props) {
  ...
  const pvToBlock = customer ? (
    <div style={{ width: '100%', ...(isEnv ? { paddingLeft: '40%' } : {}) }}>
      ...
      <div style={{ fontSize: sp.adPt * PT, lineHeight: 1.4, marginTop: 6 * PT, whiteSpace: 'pre-wrap' }}>{customer.address}</div>
      <div style={{ fontSize: sp.adPt * PT, color: '#333', marginTop: 2 * PT }}>{[customer.pin, customer.state, customer.country].filter(Boolean).join(', ')}</div>
  ...
  const pvFromEnv = showFrom ? (
    <div style={{ position: 'absolute', left: sp.padMm * MM * scale, top: 0, bottom: 0, width: '36%', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: 3 * MM }}>
      {pvFromInner}
    </div>
  ) : null
